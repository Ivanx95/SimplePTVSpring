package one.main.experimental.view;
...
public class VirtualKeyBoardLayout extends VBox {
...
//	private VBox root;
   ...
   */
  public VirtualKeyBoardLayout(ReadOnlyObjectProperty<Node> target,String text) {
...
//    this.root = new VBox(5);
  ...
  public VirtualKeyBoardLayout() {
		...
		FXMLLoader loader = new FXMLLoader(getClass().getResource("embbeded_keyboard.fxml"));
...
//	    cursorUpDown.getChildren().addAll(cursorUp, cursorDown);
	    ...
	    // "Extras" to go at the left or right end of each row of buttons.
	    final Node[][] extraLeftButtons = new Node[][] {  {tab}, {modifiers.capsLockKey()}, {modifiers.shiftKey()} };
	    final Node[][] extraRightButtons = new Node[][] { {backspace}, {delete}, {enter}, {modifiers.secondShiftKey()} };

	    // build layout
	    
	    List<HBox> rowsKeyBoard= new ArrayList<>();
	    
	    rowsKeyBoard.add(hbox1);
	    rowsKeyBoard.add(hbox2);
	    rowsKeyBoard.add(hbox3);
	    
	    Iterator<HBox> boxIterator=rowsKeyBoard.iterator();
	    for (int row = 0; row < this.unshifted.length; row++) {
	    	
	      HBox hbox = boxIterator.next();
	      hbox.getChildren().addAll(extraLeftButtons[row]);
	      for (int k = 0; k < unshifted[row].length; k++) {
	        hbox.getChildren().add( createShiftableButton(unshifted[row][k], shifted[row][k], codes[row][k], modifiers, target));
	      }
	    ...
	    hbox4.getChildren().addAll(modifiers.ctrlKey(), modifiers.altKey(),
	       /* modifiers.metaKey(),*/ spaceBar /*cursorLeft, cursorUpDown, cursorRight*/);
  }
  /**
...
//    button.getStyleClass().add("virtual-keyboard-button");
    ...
    button.setOnAction(new EventHandler<ActionEvent>() {
      ...
      public void handle(ActionEvent event) {
        ...
        if (target != null) {
          ...
          targetNode = VirtualKeyBoardLayout.this.getScene().getFocusOwner();
        }
  ...
  private static class Modifiers {
    ...
    Modifiers() {
      ...
      HBox.setHgrow(this.ctrl, Priority.NEVER);
      this.ctrl.setMaxWidth(500);
      
      HBox.setHgrow(this.alt, Priority.NEVER);
      this.alt.setMaxWidth(500);
//      tb.setMaxHeight(Double.POSITIVE_INFINITY);
    ...
    private ToggleButton createToggle(final String text) {
      ...
      tb.setMaxWidth(Double.POSITIVE_INFINITY);
      tb.setMaxHeight(Double.POSITIVE_INFINITY);
